...
def fit_one_epoch(model_train, model, yolo_loss, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen,
    ...
    with tqdm(total=epoch_step, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            if train_mode==0:
                ...
                # l2_loss = mse_loss(gaze_heatmap_pred, gaze_heatmap)
            ...
            if train_mode==0:
                ...
                total_loss =  weight_loss_value*loss_value +weight_l2_loss*l2_loss+ weight_box_energy_loss*box_energy_loss
        ...
        # lists = [loss_value_alls.item()/ (iteration + 1)]
        ...
        for iteration, batch in enumerate(gen_val):
            ...
            with torch.no_grad():
                ...
                if train_mode==0:
                    ...
                    total_loss = weight_l2_loss*l2_loss+weight_loss_value*loss_value+weight_box_energy_loss*box_energy_loss  # + Xent_loss
                    # total_loss = l2_loss + loss_value
                    loss_value_alls+=loss_value
                    l2_loss_alls+=l2_loss
                    box_energy_loss_alls+=box_energy_loss
    ...
    if train_mode==0:
        torch.save(model.state_dict(),
                   '/data2/yzd/det_gaze/data/save_model/ep%03d-loss%.3f-val_loss%.3f.pth' % (epoch + 1, loss / epoch_step, val_loss / epoch_step_val))
    if train_mode == 1:
        torch.save(model.state_dict(),

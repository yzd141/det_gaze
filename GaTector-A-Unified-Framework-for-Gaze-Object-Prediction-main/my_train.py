...
from lib.my_dataloader  import GaTectorDataset, gatector_dataset_collate
from lib.utils.utils import get_anchors, get_classes
from lib.utils.utils_fit import fit_one_epoch
param_list=["conv1","SPP","conv2","upsample1","conv_for_P4","make_five_conv1","upsample2","conv_for_P3"
    ,"make_five_conv2","yolo_head3","down_sample1","make_five_conv3","down_sample2","make_five_conv4","ps1","ps2"]
...
if __name__ == "__main__":
    ...
    # with open("./txt/gt.txt", 'r') as file:
    #     for line in file:
    #         print(1)
    #--------------------------------------------#
    ...
    #   Resume train
    # ----------------------------------------------------------------------------------------------------------------------------#
    # model_path = os.path.abspath(os.path.join(os.getcwd(),'..'))+'/GaTector-A-Unified-Framework-for-Gaze-Object-Prediction-main/model/GOO-synth ep098-loss15.975-val_loss42.955.pth'
    model_path="/data2/yzd/det_gaze/model/GOO-synth ep098-loss15.975-val_loss42.955.pth"
    # model_path = "/data2/yzd/det_gaze/model/GOO-synth ep098-loss15.975-val_loss42.955.pth"
    # model_path="/data2/yzd/det_gaze/model/ep013-loss30.587-val_loss79.297.pth"
    ...
    if model_path != '':
        ...
        for k1, v1 in pretrained_dict.items():
            k_split=k1.split(".")[0]
            if k_split in param_list:
                k_new="dethead."+k1
                new_pretrained_dict[k_new]=v1
            else:
                new_pretrained_dict[k1]=v1



        new_pretrained_dict = {k: v for k, v in new_pretrained_dict.items() if np.shape(model_dict[k]) == np.shape(v)}
    ...
    if True:
        ...
        train_dataset = GaTectorDataset(train_dir, train_annotation, input_shape, num_classes, train_mode,train=True,letterbox_image=letterbox_image)
        val_dataset = GaTectorDataset(test_dir, test_annotation, input_shape, num_classes, train_mode,train=False,letterbox_image=letterbox_image)
        ...
        if Freeze_Train:
            ...
            for param1 in model.dethead.parameters():
                param1.requires_grad =True
            for param1 in model.backbone.parameters():
                param1.requires_grad = True
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            fit_one_epoch(model, model, yolo_loss, loss_history, optimizer, epoch,
                          epoch_step, epoch_step_val, gen, gen_val, end_epoch, train_mode, Cuda)
            lr_scheduler.step()
